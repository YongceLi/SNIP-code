...
class CBTExperiment(AbstractExperiment):
    """
    ...
    def preprocess_data(self):
        ...
        def collate_fn(batch):
            """
            Given a single input in CBT, return the list of 10 different options
            """
            contexts = ''.join(batch[0]['sentences'])
            question = batch[0]['question']
            options = batch[0]['options']
            answer = batch[0]['answer']
            returned_list = []
            for choice in options:
                sentence = contexts + question.replace("XXXXX", choice)
                tokenized_sentence = self.tokenizer(sentence, return_tensors="pt")
                returned_list.append(tokenized_sentence)

            return (returned_list, options, answer)
