...
class InstrumentedModel(torch.nn.Module):
    ...
    def prune_selected_neurons(self, selected_neuron_dict):
        '''
        prune selected neurons from layers.
        selected_neuron_dict passed in as dictionary with layer names as keys and list of neuron index as values
        '''
        def prune_neurons(data, model, neuron_inds):
            assert isinstance(neuron_inds, list)
            #print(data.shape)
            data[:, :, neuron_inds] = 0.0
            return data
        
        for layer_name, neuron_inds in selected_neuron_dict.items():
            self.retain_layer(layer_name)
            self.edit_layer(layer_name, rule = prune_neurons, neuron_inds = neuron_inds)
